...
class Trigger:
    def above_value(ivs: IndicatorValues, value: float) -> bool:
        for iv in ivs:
            if iv.value > value:
                return True
...
def get_historical_candles(api_key: str, opt: CandleOption) -> List[Candle]:
    ...
    CANDLES_PER_REQ = 1150

    def to_ms(mult: int, timespan: str) -> float:
        if timespan == "hour":
    ...
    MS_PER_REQ = CANDLES_PER_REQ * to_ms(opt.mult, opt.timespan)
    ...
    while curr_start < end_unix:
        ...
        # Delay a certain amount every 4 requests
...
def export_candles(candles: List[Candle], opt: CandleOption) -> None:
    ...
    for i, c in enumerate(candles):
        ...
        row = [date, f"{c.open:.4f}", f"{c.high:.4f}", f"{c.low:.4f}", f"{c.close:.4f}", f"{c.volume:.0f}"]
        rows.append(row)
...
def export_indicators(ivs: List[IndicatorValues], name: str):
    rows = [["date", name]]

    for i, iv in enumerate(ivs):
        date = timestamp_to_datetime(iv.timestamp)
        # row = [i, date, f"{c.open:.4f}", f"{c.high:.4f}", f"{c.low:.4f}", f"{c.close:.4f}", f"{c.volume:.0f}"]
        row = [date, f"{iv.value:.4f}"]
        rows.append(row)
...
def main():
    ...
    ivs = get_indicator(API_KEY, "sma", opt)
    export_indicators(ivs, f"{ind_kind}{ind_period}")
    ...
    #     ticker="INTC"
    # )
    # candles: List[Candle] = get_historical_candles(API_KEY, opt)
    # export_candles(candles, opt)
    #
    # return
